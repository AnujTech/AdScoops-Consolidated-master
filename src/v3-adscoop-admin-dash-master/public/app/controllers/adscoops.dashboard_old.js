...
// Dashboard Box controller
  ...
  '$rootScope', '$scope', '$http', '$resource', '$interval', '$filter', 'adscoopsSocket', '$timeout','$stateParams',
  function($rootScope, $scope, $http, $resource, $interval, $filter, adscoopsSocket, $timeout,$stateParams) {
    ...
    $scope.topStats.Impressions = 0;
      ...
      adscoopsSocket.on('dailyImpressionsStats', function(payload) {
		...
		if(data.Impressions<=0){
			$scope.impressionsBilledPercent = 0;
		}else{
		  ...
		  if(data.Limit - data.Impressions<=0){
			   $scope.impressionsRequiredPercent =0;
		  }else{
    ...
    var updateDailyImpsCounts = function() {
      $resource("/adscoops/clients/viewVisible").query().$promise.then(function(clients) {
        ...
        angular.forEach($scope.clients, function(d, c) {
           ...
           // console.log(d)
            console.log("hiiii")
            console.log(d.ReceivedImps / d.RequiredImps)

            if(typeof(id)!='undefined' && id!="undefined" && d.ID==id){
                $scope.client =d;
            }

            if((d.ReceivedImps / d.RequiredImps)>0){

                d.percentage = d.ReceivedImps / d.RequiredImps;
