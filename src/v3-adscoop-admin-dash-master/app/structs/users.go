...
import (
	...
	"github.com/swhitley101/AdScoops-Consolidated/src/v3-adscoop-admin-dash-master/app/emailer"
...
func (us *Users) FindAll() error {
	return AdscoopsDB.Find(&us).Error
}
...
func (u *User) SaveJSON(user []byte) {
	...
	if err == nil {
		...
		AdscoopsDB.Table("users").Where("email = ?", uj.Email).Update(&uj)
		u = uj
	}
}
