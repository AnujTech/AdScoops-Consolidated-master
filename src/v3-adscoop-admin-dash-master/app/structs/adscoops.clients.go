...
import (
	...
	"github.com/swhitley101/AdScoops-Consolidated/src/v3-adscoop-admin-dash-master/app/billing"
...
func (c Client) RedirRealtimeStats() ClientRedirectStats {
	...
	currentTime := getNow()
	currTimePast := currentTime.Add(-30 * time.Minute).UTC().Format("2006-01-02 15:04:05")
	...
	if err := AdscoopsDB.Raw("call getRedirRealtimeStats(?,?)", currTimePast, c.ID).Scan(&retData).Error; err != nil {
		log.Errorf("Cannot grab stats: %s", err)
		return nil
	}
...
func (c Client) AssociatedRedirects() (redirects []Redirect) {
	...
	CurrentDayUTC := currentDay.UTC().Format("2006-01-02 15:04:05")
	...
	if err := AdscoopsDB.Raw("call getAssociatedRedirects(?,?)", CurrentDayUTC, c.ID).Scan(&redirects).Error; err != nil {
		log.Errorf("Cannot grab redirects: %s", err)
		return
	}
...
func (c *Client) getSpendData() (err error) {
	...
	now := time.Now().UTC()
	beginningOfMillenium := now.Add(time.Duration(-24 * time.Hour))
	log.Println("yesterday timeslice", beginningOfMillenium)
	//os.Exit(1)
